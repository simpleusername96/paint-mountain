...
func _initialize() -> void:
	...
	_assert(catalog != null and catalog.is_valid(), "current v9 catalog must be valid")
	if catalog == null:
		quit(1)
		return
	...
	for index in range(catalog.stages.size()):
		...
		_assert(stage.stage_number == number and stage.stage_version == 9, "stage version and number must match v9")
	...
	if not _failed:
		print("stage30_progression_test passed: lower/wider v9 endpoints and thirty-stage ladder")
