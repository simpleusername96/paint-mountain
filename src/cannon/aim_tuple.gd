...
var _power_percent: int


func _init(yaw: float = 0.0, elevation: float = 10.0, power: int = 0) -> void:
	_yaw_degrees = yaw
...
static func canonicalize(yaw: float, elevation: float, power: float) -> AimTuple:
	...
	return AimTuple.new(
		...
		clampi(floori(clampf(power, 0.0, 100.0) + 0.5), MINIMUM_POWER_PERCENT, MAXIMUM_POWER_PERCENT)
	)
