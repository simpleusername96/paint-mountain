...
func on_airborne_velocity(
		...
		launch_horizontal_velocity: Vector3 = Vector3.ZERO
) -> Array[Vector3]:
	...
	if horizontal.is_zero_approx():
		horizontal = Vector3(
			launch_horizontal_velocity.x,
			0.0,
			launch_horizontal_velocity.z
		) * HORIZONTAL_SPEED_MULTIPLIER
	if horizontal.is_zero_approx() or not horizontal.is_finite():
		return []
	_apex_resolved = true
